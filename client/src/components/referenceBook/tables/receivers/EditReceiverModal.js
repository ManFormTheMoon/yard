...
const EditReceiverModal = (props) => {
  const [inputValues, setInputValues] = useState(props.emptyReceiverIds);
  const onCancelEvent = () => {
    props.setVisible(false);
    setInputValues(props.emptyReceiverIds);
  };
  ...
  useEffect(async () => {
    if (props.visible == true) {
      let body = {};
      setBadFields([]);
      body.id = props.currentReceiverId;
      body = JSON.stringify(body);
      let headers = {};
      headers["Content-Type"] = "application/json";
      const response = await fetch("/api/referenceBook/receivers/getOne", {
        method: "POST",
        body: body,
        headers: headers,
      });
  ...
  const onEditEvent = async () => {
    ...
    let headers = {};
    headers["Content-Type"] = "application/json";
    const response = await fetch("/api/referenceBook/receivers/edit", {
      method: "POST",
      body: body,
      headers: headers,
    });
    ...
    if (data.message == "ok") {
     ...
     console.log("2"+err)
       if (err.includes("company_name_ru") && err.includes("Duplicate")) {
        props.showMessage(dictinary.doubleNameOfReceiver.ru);
       }
       else if (err.includes("integration_id") && err.includes("Duplicate")) {
        props.showMessage(dictinary.doubleIntegrationOfReceiver.ru);
       }
       else if (
  ...
  return (
    <Modal
      ...
      style={props.style}
    >
      <div className="title-modal">{dictinary.updateReceiver.ru}</div>
