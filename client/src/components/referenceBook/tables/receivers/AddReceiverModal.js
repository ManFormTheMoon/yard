...
const AddReceiverModal = (props) => {
  const [inputValues, setInputValues] = useState(emptyReceiverForAdd);
  ...
  const onAddEvent = async (values1) => {
    ...
    const response = await fetch("/api/referenceBook/receiver/add", {
      method: "POST",
      body: body,
      headers: headers,
    });
    ...
    if (data.message == "ok") {
      ...
      setInputValues(emptyReceiverForAdd);
      setBadFields([]);
    } else {
  ...
  return (
    <Modal
      ...
      style={props.style}
    >
      <div className="title-modal">{dictinary.addReceiver.ru}</div>
