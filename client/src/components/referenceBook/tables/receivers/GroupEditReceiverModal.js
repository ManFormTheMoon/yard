  ...
  const GroupEditReceiverModal = (props) => {
    const [inputValues, setInputValues] = useState(props.emptyReceiver);
    ...
    const onGroupEditEvent = async (checkboxValues, inputValues1) => {
      ...
      const response = await fetch("/api/referenceBook/receivers/groupEdit", {
        method: "POST",
        body: body,
        headers: headers,
      });
      ...
      if (data.message == "ok") {
        ...
        setCheckboxValues({});
        setInputValues(props.emptyReceiver);
      } else {
        setCheckboxValues({});
  ...
  export default GroupEditReceiverModal;