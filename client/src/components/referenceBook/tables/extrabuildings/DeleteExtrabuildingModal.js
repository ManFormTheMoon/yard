...
const DeleteExtrabuildingModal = (props) => {
  ...
  const onDeleteEvent = async () => {
    ...
    const response = await fetch("/api/referenceBook/extrabuilding/delete", {
      method: "POST",
      body: body,
      headers: headers,
    });
    ...
    if (data.message == "bad") {    
      const err = data.error;
       if (err.includes("foreign key")) {
        let mesError = dictinary.errorKey2.ru
...
export default DeleteExtrabuildingModal;
