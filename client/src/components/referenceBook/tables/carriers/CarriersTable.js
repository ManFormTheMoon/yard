...
import AddCarriersModal from "./AddCarriersModal";
import "../../style.css";
import EditCarrierModal from "./EditCarrierModal";
import GroupEditCarrierModal from "./GroupEditCarrierModal";
...
const CarriersTable = (props) => {
  ...
  useEffect(async () => {
    ...
    const response = await fetch("/api/referenceBook/carriers/get", {
      method: "POST",
      body: body,
      headers: headers,
    });
  ...
  const onSearchClearHandler = () => {
    setCurrentFilters(emptyCarrier);
  ...
  return (
    <>
      ...
      <AddCarriersModal
        ...
        emptyCarrierIds={emptyCarrierIds}
        showMessage={showMessage}
      />
      <EditCarrierModal
      ...
      style={style_model}
      emptyCarrierIds={emptyCarrierIds}
      currentCarrierId={selectedRows.length > 0 ? selectedRows[0] : null}
      showMessage={showMessage}
    />
      <GroupEditCarrierModal
      ...
      >
        ...
        >
          <tr>
            ...
            <td>
              <input
                ...
                onChange={(event) => onChangeInputsHandler(event, "integration_id")}
                value={currentFilters.integration_id}
              />
            </td>
            <td>
              <select
                onChange={(event) => onChangeInputsHandler(event, "booking_slot")}
                style={{ width: "95%" }}
                value={currentFilters.booking_slot}
              >
                <option></option>
            ...
            <td>
              <input
                type="text"
                style={{ width: "95%" }}
                placeholder={dictinary.blocked_time.ru}
                onChange={(event) => onChangeInputsHandler(event, "blocked_time")}
                value={currentFilters.blocked_time}
              />
            </td>
            <td>
              <input
          ...
          <tr>
            ...
            <th style={{ minWidth: "150px" }}>{dictinary.company_name.ru}</th>
            <th style={{ minWidth: "150px" }}>{dictinary.integrationCode.ru}</th>
            <th style={{ minWidth: "100px" }}>{dictinary.booking_slot.ru}?</th>
            <th style={{ minWidth: "100px" }}>{dictinary.blocked_time.ru}</th>
          ...
          {wholeData.map((cur) => {
            return (
              ...
              >
                ...
                <td>
                  {cur.booking_slot == 1 ? dictinary.yes.ru : dictinary.no.ru}
                </td>
                <td>{cur.blocked_time}</td>
                <td>{cur.comment}</td>
              </tr>
            );
          })}
