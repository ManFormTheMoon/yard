...
import GroupEditSupplierModal from "./GroupEditSupplierModal";
...
const SuppliersTable = (props) => {
  ...
  useEffect(async () => {
    ...
    const response = await fetch("/api/referenceBook/suppliers/get", {
      method: "POST",
      body: body,
      headers: headers,
    });
  ...
  const onSearchClearHandler = () => {
    setCurrentFilters(emptySupplier);
  ...
  return (
    <>
      ...
      <AddSupplierModal
        ...
        emptySupplierIds={emptySupplierIds}
        showMessage={showMessage}
      />
      <EditSupplierModal
        ...
        style={style_model}
        emptySupplierIds={emptySupplierIds}
        currentSupplierId={selectedRows.length > 0 ? selectedRows[0] : null}
        showMessage={showMessage}
      />
     <GroupEditSupplierModal
