  ...
  const GroupEditSupplierModal = (props) => {
    const [inputValues, setInputValues] = useState(props.emptySupplier);
    ...
    const onGroupEditEvent = async (checkboxValues, inputValues1) => {
      ...
      const response = await fetch("/api/referenceBook/suppliers/groupEdit", {
        method: "POST",
        body: body,
        headers: headers,
      });
      ...
      if (data.message == "ok") {
        ...
        setCheckboxValues({});
        setInputValues(props.emptySupplier);
      } else {
        setCheckboxValues({});
    ...
    return (
      ...
      >
        ...
        <div className={`${"row-styles"} ${"not-edit"}`}>
          <div style={rowName}>
            {dictinary.company_name.ru}:<span> *</span>
          </div>
  ...
  export default GroupEditSupplierModal;