...
import GroupEditAreaModal from "./GroupEditAreaModal";
...
const AreasTable = (props) => {
  ...
  useEffect(async () => {
    ...
    const response = await fetch("/api/referenceBook/areas/get", {
      method: "POST",
      body: body,
      headers: headers,
    });
  ...
  return (
    <>
      ...
      style={style_model}
      emptyAreaIds={emptyAreaIds}
      currentAreaId={selectedRows.length > 0 ? selectedRows[0] : null}
      showMessage={showMessage}
    />
      <GroupEditAreaModal
