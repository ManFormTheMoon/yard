...
import GroupEditCheckpointModal from "./GroupEditCheckpointModal";
...
const CheckpointsTable = (props) => {
  ...
  useEffect(async () => {
    ...
    const response = await fetch("/api/referenceBook/checkpoints/get", {
      method: "POST",
      body: body,
      headers: headers,
    });
  ...
  const onSearchClearHandler = () => {
    setCurrentFilters(emptyCheckpoint);
  ...
  return (
    <>
      ...
      <AddCheckpointModal
        ...
        emptyCheckpointIds={emptyCheckpointIds}
        showMessage={showMessage}
      />
      <EditCheckpointModal
      ...
      style={style_model}
      emptyCheckpointIds={emptyCheckpointIds}
      currentCheckpointId={selectedRows.length > 0 ? selectedRows[0] : null}
      showMessage={showMessage}
    />
      <GroupEditCheckpointModal
