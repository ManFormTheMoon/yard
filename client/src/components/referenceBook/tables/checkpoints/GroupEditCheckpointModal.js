...
const GroupEditCheckpointModal = (props) => {
  const [inputValues, setInputValues] = useState(props.emptyCheckpoint);
  ...
  const onGroupEditEvent = async (checkboxValues, inputValues1) => {
    ...
    const response = await fetch("/api/referenceBook/checkpoints/groupEdit", {
      method: "POST",
      body: body,
      headers: headers,
    });
    ...
    if (data.message == "ok") {
      ...
      setCheckboxValues({});
      console.log("x");
      setInputValues(props.emptyCheckpoint);
    } else {
      setCheckboxValues({});
...
export default GroupEditCheckpointModal;
