...
const AddAreaModal = (props) => {
  const [inputValues, setInputValues] = useState(emptyCheckpointForAdd);
  ...
  const onAddEvent = async (values1) => {
    ...
    const response = await fetch("/api/referenceBook/checkpoints/add", {
      method: "POST",
      body: body,
      headers: headers,
    });
    ...
    if (data.message == "ok") {
      ...
      setInputValues(emptyCheckpointForAdd);
      setBadFields([]);
    } else {
  ...
  return (
    <Modal
      ...
      style={props.style}
    >
      <div className="title-modal">{dictinary.addCheckpoint.ru}</div>
      <div className="row-styles">
        ...
        <input
          ...
          placeholder= {dictinary.enterNameOfCheckpoint.ru}
